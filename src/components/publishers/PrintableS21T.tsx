...
export function PrintableS21T({ publisher, reports, serviceYear }: Props) {
  ...
  return (
    <div
      className="hidden print:block w-[210mm] min-h-[297mm] mx-auto bg-white text-black pt-8 px-10 font-sans text-[13px]"
    ...
    >
          ...
          <div className="flex items-center gap-4 text-[12px] font-bold">
            <label className="flex items-center gap-1">
              <div className="w-[14px] h-[14px] border border-black flex items-center justify-center bg-white">
                {publisher.gender === 'Masculino' && <Check className="w-3 h-3" strokeWidth={3} />}
              </div>
              Masculino
            </label>
            <label className="flex items-center gap-1">
      ...
      <div className="flex flex-wrap items-center justify-between mb-4 mt-2 text-[12px] font-bold px-1">
        ...
        <label className="flex items-center gap-1">
          <div className="w-[14px] h-[14px] border border-black flex items-center justify-center">
            {publisher.type === 'pioneiro_regular' && <Check className="w-3 h-3" strokeWidth={3} />}
          </div>
          Pioneiro regular
        </label>
        <label className="flex items-center gap-1">
      ...
      <table className="w-full border-collapse border border-black text-center text-[12px]">
        <thead>
          <tr className="bg-[#e6ebf5]">
            <th className="border border-black p-1 font-bold w-[90px] leading-tight">
              Ano de serviço
              <br />
              <span className="text-[14px]">{serviceYear}</span>
            </th>
            <th className="border border-black p-1 font-bold w-[80px] leading-tight">
              Participou
              <br />
              no
              <br />
            ...
            <th className="border border-black p-1 font-bold w-[70px] leading-tight">
              Estudos
              <br />
              bíblicos
            </th>
            ...
            <th className="border border-black p-1 font-bold">Observações</th>
          </tr>
        </thead>
        <tbody>
          {SERVICE_MONTHS.map(({ month, name }) => {
            ...
            return (
              <tr key={month} className="h-[24px]">
                ...
                <td className="border border-black p-1 text-left px-2 bg-[#e6ebf5]/30">
                  {rep?.notes || ''}
                </td>
              </tr>
            )
      ...
      <div className="mt-[200px] text-[10px] text-gray-800 font-bold tracking-tight">
        S-21-T 11/23
      </div>
    </div>
  )
